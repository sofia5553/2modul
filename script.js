...
function updateCartCount() {
    if (cartInfo) {
        ...
        if (countEl) {
            ...
            setTimeout(() => countEl.classList.remove('bump'), 340);
        } else {
    ...
    productsArray.forEach(function (product) {
        ...
        card.innerHTML = `
            ...
            <div class="card-footer">
                ...
                <button class="add-to-cart-btn" aria-label="Добавить ${escapeHtml(product.title)} в корзину">
                    <span class="btn-icon" aria-hidden="true">
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M7 4H3v2h2l3.6 7.59-1.35 2.44A1 1 0 007 17H19v-2H8.42a.25.25 0 01-.24-.15L9 13h7a1 1 0 00.9-.55L20.9 7H7z" fill="currentColor"/></svg>
                    </span>
...
function filterAndRender() {
    ...
    if (searchWrapper) {
        const countEl = searchWrapper.querySelector('.search-count');
        if (countEl) {
    ...
    if (resultsInfo) {
        ...
        if (resultsPulseTimer) clearTimeout(resultsPulseTimer);
        resultsInfo.classList.remove('pulse');
        void resultsInfo.offsetWidth;
        resultsInfo.classList.add('pulse');
        resultsPulseTimer = setTimeout(() => resultsInfo.classList.remove('pulse'), 700);
    }
}
loadCart();
updateCartCount();
...
if (productsContainer) {
    filterAndRender();
    if (searchInput) {
        searchInput.addEventListener("input", function (e) { filterAndRender(); toggleSearchClear(); });
        toggleSearchClear();
    }
    ...
    if (clearSearchBtn) {
        clearSearchBtn.addEventListener('click', function () { if (searchInput) { searchInput.value = ''; filterAndRender(); searchInput.focus(); toggleSearchClear(); } });
    }
}
